...
export default function SellerLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (isAuthenticated && canHaveShop && !isRegisterPage) {
      dispatch(getMyShop());
    }
  }, [dispatch, isAuthenticated, canHaveShop, isRegisterPage]);
          ...
          <div className="bg-[#f7f7f7] rounded-xl p-4 mb-4">
            <div className="flex items-center gap-3">
              <div className="w-14 h-14 rounded-full overflow-hidden bg-white">
                {myShop?.logo ? (
                  <Image src={myShop.logo} alt={myShop.name} width={56} height={56} className="object-cover" />
                ) : (
              ...
              <div className="min-w-0 flex-1">
                <p className="font-semibold text-gray-800 truncate">{myShop?.name}</p>
                <div className="flex items-center gap-1.5 mt-1">
                  <span className={`w-2 h-2 rounded-full ${myShop?.status === "active" ? "bg-green-500" : "bg-gray-400"}`} />
