...
import SpinnerLoading from "../common/SpinerLoading";
...
export default function OrdersTab() {
  ...
  const { userOrders, isLoading, error } = useAppSelector((state) => state.order);
  ...
  const statusTabs: { value: OrderStatus; label: string; count: number }[] = [
    ...
    { value: "pending", label: "Chờ xác nhận", count: getOrderCount("pending") },
    { value: "confirmed", label: "Đã xác nhận", count: getOrderCount("confirmed") },
    { value: "processing", label: "Đang xử lý", count: getOrderCount("processing") },
  ...
  // Loading Skeleton
  if (isLoading && (!userOrders || userOrders.length === 0)) {
          ...
          {filteredOrders.length === 0 ? (
            <div className="text-center py-12 border rounded-lg">
              ...
              <h3 className="text-lg font-semibold mb-2">
                {activeStatus === "all" ? "Chưa có đơn hàng" : `Không có đơn hàng ${statusTabs.find(tab => tab.value === activeStatus)?.label.toLowerCase()}`}
              </h3>
              <p className="text-muted-foreground mb-6">
                {activeStatus === "all"
                  ? "Lịch sử đơn hàng của bạn sẽ xuất hiện tại đây"
