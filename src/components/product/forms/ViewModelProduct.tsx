...
import { Product, Category, ShopCategory } from "@/types/product";
import { 
  Package, Tag, Box, Calendar, Store, ImageIcon, 
  Edit, ExternalLink, Layers, Palette, Ruler, 
  Weight, ShoppingBag, Star, TrendingUp
} from "lucide-react";
...
function getColorCode(colorName: string): string {
  const colorMap: Record<string, string> = {
    ...
    'xanh': '#1976D2', 'blue': '#1976D2', 'xanh dương': '#1976D2',
    'xanh lá': '#43A047', 'green': '#43A047',
    'vàng': '#FDD835', 'yellow': '#FDD835',
    'cam': '#FB8C00', 'orange': '#FB8C00',
    'tím': '#8E24AA', 'purple': '#8E24AA',
    'hồng': '#EC407A', 'pink': '#EC407A',
    'đen': '#212121', 'black': '#212121',
    'trắng': '#FAFAFA', 'white': '#FAFAFA',
    'xám': '#757575', 'gray': '#757575', 'grey': '#757575',
    'nâu': '#795548', 'brown': '#795548',
    'be': '#D7CCC8', 'beige': '#D7CCC8',
...
export function ViewModelProduct({
  ...
  const getStatusConfig = (status?: string) => {
    switch (status) {
      ...
      case "draft":
        return { label: "Bản nháp", className: "bg-amber-50 text-amber-700 border-amber-200" };
      case "suspended":
        return { label: "Tạm ngưng", className: "bg-red-50 text-red-700 border-red-200" };
      default:
        return { label: "Không xác định", className: "bg-gray-50 text-gray-600 border-gray-200" };
    }
  };
        ...
        <DialogHeader className="px-6 py-4 border-b bg-gradient-to-r from-gray-50 to-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="text-lg font-bold">Chi tiết sản phẩm</DialogTitle>
                <p className="text-xs text-gray-500 mt-0.5">ID: {product._id}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Badge variant="outline" className={`rounded-full px-3 py-1 text-xs font-medium ${statusConfig.className}`}>
                {statusConfig.label}
              </Badge>
              <Link href={`/products/${product.slug}`} target="_blank">
                <Button variant="ghost" size="sm" className="h-8 px-2 text-gray-500 hover:text-[#E53935]">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex gap-6 p-5 bg-gradient-to-br from-gray-50 to-white rounded-2xl border">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-400 font-mono mt-1">/{product.slug}</p>
                
                {/* Tags */}
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {product.isNewArrival && (
                    <Badge className="bg-blue-50 text-blue-600 border-blue-200 text-xs">Mới</Badge>
                  )}
                  {product.isFeatured && (
                    <Badge className="bg-purple-50 text-purple-600 border-purple-200 text-xs">Nổi bật</Badge>
                  )}
                  {product.onSale && (
                    <Badge className="bg-red-50 text-red-600 border-red-200 text-xs">Giảm giá</Badge>
                  )}
                  {product.brand && (
                ...
                <div className="flex items-center gap-4 mt-4">
                  <div className="flex items-center gap-1.5 text-sm">
                    ...
                    <span className="font-medium">{product.ratingAverage?.toFixed(1) || "0.0"}</span>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="p-4 rounded-xl bg-white border hover:shadow-sm transition-shadow">
                <div className="flex items-center gap-2 text-gray-400 mb-3">
                  ...
                  <span className="text-xs font-medium uppercase tracking-wide">Danh mục</span>
                </div>
                <p className="font-semibold text-gray-800">{getCategoryName(product.category)}</p>
                {shopCategoryName && (
              ...
              <div className="p-4 rounded-xl bg-white border hover:shadow-sm transition-shadow">
                <div className="flex items-center gap-2 text-gray-400 mb-3">
                  ...
                  <span className="text-xs font-medium uppercase tracking-wide">Tồn kho</span>
                </div>
                <p className={`font-semibold text-2xl ${
                  getTotalStock() > 10 ? 'text-emerald-600' : 
                  getTotalStock() > 0 ? 'text-amber-600' : 'text-red-500'
                }`}>
                  {getTotalStock()}
                </p>
              ...
              <div className="p-4 rounded-xl bg-white border hover:shadow-sm transition-shadow">
                ...
                <p className="text-sm text-gray-600">
                  Tạo: <span className="font-medium">{new Date(product.createdAt).toLocaleDateString("vi-VN")}</span>
                </p>
                <p className="text-sm text-gray-600 mt-1">
                  Sửa: <span className="font-medium">{new Date(product.updatedAt).toLocaleDateString("vi-VN")}</span>
                </p>
              </div>
            </div>
            ...
            {product.sizes && product.sizes.length > 0 && (
              <div className="p-4 rounded-xl bg-white border">
                <div className="flex items-center gap-2 text-gray-400 mb-3">
                  ...
                  <span className="text-xs font-medium uppercase tracking-wide">Kích thước có sẵn</span>
                </div>
              ...
              <div className="rounded-xl border overflow-hidden">
                ...
                <div className="divide-y">
                  {product.variants.map((variant, idx) => (
                    <div key={variant._id || idx} className="p-4 hover:bg-gray-50/50 transition-colors">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {variant.sku && (
                            <p className="text-xs text-gray-400 font-mono mt-1">SKU: {variant.sku}</p>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-6 text-sm">
                          <div className="text-center">
                            ...
                            <p className="font-semibold text-[#E53935]">{variant.price?.toLocaleString()}₫</p>
                          </div>
                          <div className="text-center">
                            <p className="text-gray-400 text-xs">Tồn kho</p>
                            <p className={`font-semibold ${
                              variant.stock > 10 ? 'text-emerald-600' : 
                              variant.stock > 0 ? 'text-amber-600' : 'text-red-500'
                            }`}>{variant.stock}</p>
                          </div>
                          <div className="text-center">
                            <p className="text-gray-400 text-xs">Đã bán</p>
                            <p className="font-medium text-gray-600">{variant.sold || 0}</p>
                          </div>
                          <div className="text-center">
                            <p className="text-gray-400 text-xs">Ảnh</p>
                            <p className="font-medium text-gray-600">{variant.images?.length || 0}</p>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            {product.description && (
              <div className="p-4 rounded-xl bg-white border">
                <div className="flex items-center gap-2 text-gray-400 mb-3">
                  ...
                  <span className="text-xs font-medium uppercase tracking-wide">Mô tả sản phẩm</span>
                </div>
            ...
            {(product.weight || product.dimensions) && (
              <div className="p-4 rounded-xl bg-white border">
                <div className="flex items-center gap-2 text-gray-400 mb-3">
                  ...
                  <span className="text-xs font-medium uppercase tracking-wide">Thông tin vận chuyển</span>
                </div>
                <div className="flex gap-6">
                  {product.weight && (
                    <div className="flex items-center gap-2">
                      <Weight className="w-4 h-4 text-gray-400" />
                      <span className="text-sm text-gray-600">Cân nặng: <span className="font-medium">{product.weight}g</span></span>
                    </div>
                  )}
                  {product.dimensions && (product.dimensions.length || product.dimensions.width || product.dimensions.height) && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-600">
                        Kích thước: <span className="font-medium">
            ...
            {product.attributes && product.attributes.length > 0 && (
              <div className="p-4 rounded-xl bg-white border">
                <div className="flex items-center gap-2 text-gray-400 mb-3">
                  <Box className="w-4 h-4" />
                  <span className="text-xs font-medium uppercase tracking-wide">Thông số kỹ thuật</span>
                </div>
              ...
              <div className="p-4 rounded-xl bg-white border">
                ...
                <div className="flex flex-wrap gap-2">
                  {product.tags.map((tag) => (
                    <Badge key={tag} variant="secondary" className="bg-gray-100 text-gray-600 rounded-full px-3">
                      {tag}
                    </Badge>
                  ))}
                </div>
              ...
              <div className="p-4 rounded-xl bg-white border">
                ...
                <div className="grid grid-cols-6 gap-2">
                  {allImages.slice(0, 12).map((img, idx) => (
                    <div key={idx} className="relative aspect-square rounded-lg overflow-hidden border hover:border-[#E53935] transition-colors">
